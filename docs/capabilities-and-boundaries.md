...
这份文档补充 README 的项目亮点。README 只放首页级介绍；如果想了解 Friday 为什么适合长期运行、哪些事情有安全边界、哪些行为可以配置，就看这里。

## 项目定位

Friday 是一个个人 AI Agent runtime，不是单一聊天界面。它把模型调用、工具执行、权限、安全、记忆、平台 Gateway、MCP、workflow、sub-agent、多模态和运行态观测放到同一个后端核心里。
...
Provider 不只是 `base_url` 和 `model`。Friday 会根据 provider profile 和 API mode 选择 transport，并处理不同协议的请求格式：
...
Friday 的安全设计分两层：可配置权限和不可轻易绕过的硬边界。
...
Friday 的目标是尽量通过配置切换运行方式，而不是改代码。
...
Friday 不是为了让模型无限制接管机器。更准确地说，它是在安全边界内给模型自动化能力：
