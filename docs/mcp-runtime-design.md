...
Friday 将 MCP 从一次性 stdio 工具加载器演进为长期运行的外部能力 runtime，同时保留现有工具权限、安全、审计和渐进式披露语义。

## 决策

- 使用官方 MCP Python SDK 的稳定 v1.x 协议与 transport 实现，依赖约束为 `mcp>=1.27,<2`。
- Friday 自己管理 server 生命周期、重连、工具快照、Registry 同步、健康状态和安全策略。
- SDK 类型只存在于 MCP connection adapter 内部；Manager、runtime 和工具层使用 Friday 自己的模型。
