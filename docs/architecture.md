...
这份文档对应 README 里的 `Runtime Flow`，用于帮助开发者理解 Friday 内部怎么从“用户输入”流转到“模型调用、工具执行、持久化和观测”。
...
- `memory/lumora` 使用百炼 OpenAI-compatible embedding、Qdrant 语义检索和 SQLite FTS5/BM25，并以 RRF 融合结果。
...
用户需要安装和配置 MCP server；Friday 负责启动、连接、列工具、执行工具和纳入权限链路。
