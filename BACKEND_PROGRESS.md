...
- Python 包名 `personal_agent`、CLI `personal-agent`，以及 `memory/lumora` 等技术标识保持不变。
- 路线图文件重命名为 `friday-roadmap.zh-CN.md`；代码与运行入口未改。

## 2026-07-15：AD-044 统一持久化脱敏

- 新增 `sanitize_persistence_*` / `sanitize_tool_run_for_persistence` 作为唯一 persistence sanitizer；`clean_*` 仅做 UTF-8 清理。
- 字段分类：`public` / `sensitive` / `secret` / `debug-opt-in`；secret 键值写为 `[REDACTED]`，artifact `data`/`payload` 等 debug 字段默认不落库。
- 收敛写入出口：`save_message`、`save_tool_runs`、`save_turn_report`、`export_jsonl`、audit JSONL；`full_output` 默认截断 8000 字符并脱敏。
- ConversationService 内存近期 tool runs 与 `/tool-runs show` query 投影同步走安全摘要，避免绕过 SQLite 边界读到明文。
- `redact()` 补齐 Bearer、URL query secret、Cookie 与 `token=` 赋值；保留 `session_id` / `cache_hit_tokens` 等诊断字段。
- 回归：`tests/test_persistence_sanitizer.py`、`tests/test_database.py` 及相关 conversation/commands/tool_pipeline 聚焦套件 `117 passed`。
- 未做：加密原始证据仓、既有 `state.db` 一次性扫描/迁移告警。
...
- 修正 doctor 对 Streamable HTTP MCP 的误报；普通 `uv run personal-agent doctor --verbose` 已验证 GitHub `runtime=ready`、44 个工具，`memory/lumora` 正常，不再需要 `--env-file` 启动参数。
- 测试隔离补充：runtime 单测显式关闭真实 external memory，避免继承本机 `memory/lumora`/Qdrant 配置。
...
- fallback observation 可由 `MemoryReviewService` 后台批量迁回 `memory/lumora`，不占用 Agent 主循环；迁移失败保留 attempts/error，后续继续恢复。
- `memory/lumora` 会探测并修复 Qdrant collection/vector/payload index；本轮真实运行中 embedding 与 Qdrant query 均成功，当前微信 scope 保持 `lumora`，已有 pending observation 成功迁移。
...
- MCP client 切换到官方 Python SDK 稳定 v1.x，协议与 transport 封装在 Friday connection contract 后，保留旧 stdio 配置兼容。
...
- 根目录新增架构路线图文档（现为 `friday-roadmap.zh-CN.md`），将后续架构路线图整理为中文；移除已完成的 provider/transport 实施指令和前端历史清理计划，并修正 `CLAUDE.md` 的过时链接与 provider/transport 描述。
...
- Project display rename：对外展示名从 `Personal Agent` 调整为 `Friday`；内部 Python 包名 `personal_agent` 和 CLI 命令 `personal-agent` 暂时保留，避免破坏运行入口。
...
- `memory/lumora` 返回实际落库后的 `memory_id/content/previous_content`，并记录 search、resolve、apply、embedding、Qdrant 各阶段耗时。
...
- memories schema v4 记录逐条索引尝试次数、最近错误和更新时间；`memory/lumora` worker 每次后台修复一条 pending index。
...
真实配置验证：probe 状态为 `ok`，`memory/lumora` 可用；诊断准确报告全局 migration pending `59`、index pending `1`，未在前台自动迁移这些数据。
