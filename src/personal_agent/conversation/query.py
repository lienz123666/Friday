...
from personal_agent.text_safety import sanitize_tool_run_for_persistence
...
def _normalize_tool_run(item: dict[str, Any]) -> dict[str, Any]:
    # Re-project at the query boundary so legacy rows and in-memory caches
    # cannot surface plaintext secrets via /tool-runs show.
    safe = sanitize_tool_run_for_persistence(item)
    return {
        "id": int(item.get("id") or 0),
        "session_id": str(safe.get("session_id") or ""),
        "session_key": str(safe.get("session_key") or ""),
        "turn_id": str(safe.get("turn_id") or ""),
        "tool_use_id": str(safe.get("tool_use_id") or ""),
        "tool_name": str(safe.get("tool_name") or ""),
        "status": str(safe.get("status") or item.get("status") or ""),
        "category": str(safe.get("category") or item.get("category") or ""),
        "duration": float(item.get("duration") or 0.0),
        "input_summary": str(safe.get("input_summary") or ""),
        "output_summary": str(safe.get("output_summary") or ""),
        "full_output": str(safe.get("full_output") or ""),
        "output_truncated": bool(safe.get("output_truncated", item.get("output_truncated", False))),
