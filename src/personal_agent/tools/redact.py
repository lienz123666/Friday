...
_URL_SECRET_RE = re.compile(
    r'([?&](?:access[_-]?token|api[_-]?key|apikey|auth(?:entication)?|'
    r'token|secret|password|passwd|session(?:_id)?|cookie)=)([^&#\s]+)',
    re.IGNORECASE,
)
_COOKIE_HEADER_RE = re.compile(r'\b(Set-Cookie|Cookie)\s*:\s*[^\r\n]+', re.IGNORECASE)
_SECRET_ASSIGN_RE = re.compile(
    r'\b((?:access[_-]?token|api[_-]?key|apikey|auth(?:entication)?|'
    r'token|secret|password|passwd|session(?:_id)?|cookie))\s*([:=])\s*'
    r'([^\s,;]+)',
    re.IGNORECASE,
)
...
def redact(text: str) -> str:
    ...
    # Bearer values and URL query credentials need dedicated handling because
    # the generic header expression intentionally stops at whitespace.
    text = _BEARER_RE.sub(_replace_bearer, text)
    text = _URL_SECRET_RE.sub(_replace_url_secret, text)
    text = _COOKIE_HEADER_RE.sub(r'\1: [REDACTED]', text)

    # Bare secret assignments such as `token=...` outside URL query strings.
    text = _SECRET_ASSIGN_RE.sub(_replace_secret_assign, text)
...
def _replace_bearer(m: re.Match) -> str:
    return f"Bearer {_mask_token(m.group(1))}"


def _replace_url_secret(m: re.Match) -> str:
    return f"{m.group(1)}[REDACTED]"


def _replace_secret_assign(m: re.Match) -> str:
    value = m.group(3)
    # Avoid re-masking values already replaced by URL/cookie handlers.
    if value == "[REDACTED]" or (value and set(value) <= {"*"}):
        return m.group(0)
    return f"{m.group(1)}{m.group(2)}{_mask_token(value)}"
