...
class Database:
    ...
    async def save_tool_runs(self, runs: list[dict[str, Any]]) -> int:
        ...
        rows = []
        for raw_run in runs:
            run = sanitize_tool_run_for_persistence(raw_run)
            rows.append((
                ...
                str(run.get("full_output") or ""),
                1 if run.get("output_truncated") else 0,
                json.dumps(run.get("artifacts") or [], ensure_ascii=False, sort_keys=True),
                json.dumps(run.get("result_metadata") or {}, ensure_ascii=False, sort_keys=True),
    ...
    async def save_turn_report(self, envelope: dict[str, Any]) -> int:
        ...
        row = (
            ...
            sanitize_persistence_text(str(report.get("error") or ""), max_chars=2_000),
            sanitize_persistence_text(str(report.get("user_message_summary") or ""), max_chars=2_000),
            sanitize_persistence_text(str(report.get("final_response_summary") or ""), max_chars=2_000),
            ...
            json.dumps(sanitize_persistence_payload(source, max_string_chars=2_000), ensure_ascii=False, sort_keys=True),
            json.dumps(sanitize_persistence_payload(report, max_string_chars=2_000), ensure_ascii=False, sort_keys=True),
