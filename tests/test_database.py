...
def test_persistence_redacts_tool_outputs_metadata_reports_and_exports(db, tmp_path: Path):
    sid = str(uuid.uuid4())
    session_key = "cli:default:local"
    api_key = "sk-proj-abcdefghijklmnopqrstuvwxyz"
    bearer_token = "bearer-secret-abcdefghijklmnopqrstuvwxyz"
    query_token = "query-secret-abcdefghijklmnopqrstuvwxyz"
    artifact_payload = "artifact-private-payload"
    _run(db.create_session_direct(sid, session_key))

    _run(db.save_message(
        sid,
        "user",
        content=f"save {api_key}",
        tool_calls=[{"name": "demo", "input": {"api_key": api_key}}],
    ))
    _run(db.save_tool_runs([{
        "session_id": sid,
        "session_key": session_key,
        "tool_use_id": "secret-run",
        "tool_name": "demo",
        "status": "success",
        "input_summary": f"Authorization: Bearer {bearer_token}",
        "output_summary": f"https://example.test/?access_token={query_token}",
        "full_output": f"key={api_key}; Authorization: Bearer {bearer_token}",
        "artifacts": [{
            "kind": "file",
            "name": "result.txt",
            "data": artifact_payload,
            "metadata": {"token": bearer_token},
        }],
        "result_metadata": {
    ...
    _run(db.save_turn_report({
        "session_id": sid,
        "session_key": session_key,
        "source": {"authorization": f"Bearer {bearer_token}"},
        "report": {
            "turn_id": "secret-turn",
            "status": "completed",
            "user_message_summary": f"token={query_token}",
            "final_response_summary": f"key={api_key}",
            "diagnostics": {"cookie": "session=private"},
        },
    }))

    async def raw_persistence() -> str:
        rows = await db._conn.execute(
            "SELECT content, tool_calls FROM messages WHERE session_id = ?",
            (sid,),
        )
        messages = [dict(row) async for row in rows]
        rows = await db._conn.execute(
            "SELECT full_output, artifact_summary_json, result_metadata_json FROM tool_runs "
            "WHERE session_id = ?",
            (sid,),
        )
        tool_runs = [dict(row) async for row in rows]
        rows = await db._conn.execute(
            "SELECT source_json, report_json FROM turn_reports WHERE session_id = ?",
            (sid,),
        )
        reports = [dict(row) async for row in rows]
        return json.dumps([messages, tool_runs, reports], ensure_ascii=False)

    raw = _run(raw_persistence())
    for secret in (api_key, bearer_token, query_token):
        assert secret not in raw
    assert artifact_payload not in raw
    assert "[REDACTED]" in raw

    run = _run(db.recent_tool_runs(limit=1, session_key=session_key))[0]
    assert run["result_metadata"]["nested"]["api_key"] == "[REDACTED]"
    assert run["result_metadata"]["safe_flag"] is True
    assert run["artifacts"] == [{
        "has_data": True,
        "kind": "file",
        "name": "result.txt",
    }]
    assert "data" not in run["artifacts"][0]
    assert artifact_payload not in json.dumps(run, ensure_ascii=False)

    export_path = tmp_path / "session.jsonl"
    _run(db.export_jsonl(sid, str(export_path)))
    exported = export_path.read_text(encoding="utf-8")
    assert api_key not in exported
    assert bearer_token not in exported
