...
- 执行模式：`config.yaml -> execution.mode`，交互临时切换用 `/mode`。
- 系统提示词：`data/system/*.md` 与 `profiles` session 映射。
- 运行数据：`data/`，不进入 git。
- 安全边界：sandbox roots、blocked patterns、bash network、write limit、audit。
- 可观测性：doctor、runtime health、tool runs、turn reports、activity、cache diagnostics。
